...
FormData.prototype.deleteAll_=function () {
    var _this=this;
...
function upload(){
    ...
    var data_=Dragfiles(); //获取formData
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(obj['state'] == 'success'){
                    var url = 'go_config?spectrum=' + $('#selectedItem option:selected') .text();
                    window.open(url)
                }else if(obj['state'] == 'fail'){
                    toastr.error(obj['msg'])
                }else if(obj['state'] == 'error'){
                    toastr.error('Upload files failed')
                }

            },
            error: function (returndata) {
                data_.deleteAll_();
                toastr.error('Upload files failed!')  //可以替换为自己的方法
            }
        });
    }else{
        $.ajax({
            ...
            error: function (returndata) {
                toastr.error('failed!')  //可以替换为自己的方法
            }
        });
    }
}
